(() => {
  ...
  function cleanUrl(value) {
    ...
    if (!/^https?:$/.test(u.protocol) || u.username || u.password) {
      throw new Error('Use a normal public http/https URL.');
    }
    u.hash = '';
    return u.toString();
  }
  ...
  function addReady(results) {
    ...
    try {
      ...
      collection = raw === null
        ? { revision: 0, sources: [], library: [], progress: {} }
        : JSON.parse(raw);
    } catch {
      throw new Error('Saved collection could not be read.');
    }

    if (!collection || typeof collection !== 'object' || !Array.isArray(collection.sources)) {
      throw new Error('Saved collection could not be read.');
    }
    ...
    const idsSeenThisRun = new Set();

    for (const item of results) {
      const extension = item.extension;
      if (!extension?.id || !extension?.name) {
        outcome.set(item.target, 'failed');
        continue;
      }

      const id = 'yomuext-' + extension.id;
      if (idsSeenThisRun.has(id)) {
        outcome.set(item.target, 'already');
        continue;
      }
  ...
  function makeCss() {
    ...
    style.textContent = `
      ...
      #${PACK_ID}{display:none;margin-top:12px}
      #${PACK_ID}.is-active{display:block}
      ...
      #${PACK_ID} .sp-meta{display:flex;justify-content:space-between;gap:12px;align-items:center;margin:0 0 8px;color:var(--dim,#8297aa);font-size:10.5px}
      #${PACK_ID} .sp-meta b{color:var(--muted,#91a8bb);font-size:11px}
      #${PACK_ID} .sp-actions{display:flex;align-items:center;gap:9px;margin-top:10px;flex-wrap:wrap}
      #${PACK_ID} .sp-run{height:44px;background:var(--accent,#ffc15a);color:var(--accentText,#0c131b)}
      #${PACK_ID} .sp-clear{height:44px;background:transparent;color:var(--muted,#91a8bb);border:1px solid var(--line,#263747)}
      #${PACK_ID} .sp-summary{font-size:11.5px;color:var(--muted,#91a8bb);margin-left:auto}
      #${PACK_ID} .sp-progress{height:4px;border-radius:999px;background:color-mix(in srgb,var(--line,#263747) 75%,transparent);overflow:hidden;margin-top:11px;display:none}
      #${PACK_ID} .sp-progress.show{display:block}#${PACK_ID} .sp-progress>i{display:block;height:100%;width:0;background:var(--accent,#ffc15a);transition:width .18s ease}
      #${PACK_ID} .sp-results{display:grid;gap:7px;margin-top:12px}
      #${PACK_ID} .sp-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:10px;align-items:center;border:1px solid var(--hairline,var(--line,#263747));border-radius:12px;padding:9px 10px;background:color-mix(in srgb,var(--bg,#09111a) 55%,transparent)}
      #${PACK_ID} .sp-site{min-width:0}#${PACK_ID} .sp-site b{display:block;font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}#${PACK_ID} .sp-site small{display:block;margin-top:2px;font-size:10.5px;color:var(--dim,#8297aa);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
      #${PACK_ID} .sp-state{font-size:10.5px;font-weight:800;border-radius:999px;padding:5px 8px;border:1px solid var(--line,#263747);white-space:nowrap;color:var(--dim,#8297aa)}
      #${PACK_ID} .sp-row[data-state="working"] .sp-state{color:var(--accent,#ffc15a)}
      #${PACK_ID} .sp-row[data-state="added"] .sp-state,#${PACK_ID} .sp-row[data-state="ready"] .sp-state{color:#91e4ad;border-color:rgba(120,220,160,.35)}
      #${PACK_ID} .sp-row[data-state="already"] .sp-state{color:#9ec7ff}
      #${PACK_ID} .sp-row[data-state="browser"] .sp-state{color:#ffc86e}
      #${PACK_ID} .sp-row[data-state="failed"] .sp-state,#${PACK_ID} .sp-row[data-state="blocked"] .sp-state{color:#ff8a81;border-color:rgba(255,120,110,.3)}
  ...
  function mount() {
    ...
    makeCss();
    const headerCopy = root.querySelector('.sf-copy');
    ...
    const singleTitle = title.textContent;
    const singleCopy = headerCopy.textContent;

    const switcher = document.createElement('div');
    switcher.className = 'sf-mode-switch';
    ...
    pack.id = PACK_ID;
    pack.innerHTML = `
      <div class="sp-meta"><b>Paste one site per line</b><span>Up to ${MAX_URLS} sites · ${CONCURRENCY} tests at once</span></div>
    ...
    const runButton = pack.querySelector('.sp-run');
    const clearButton = pack.querySelector('.sp-clear');
    const summary = pack.querySelector('.sp-summary');
    const progress = pack.querySelector('.sp-progress');
    const progressBar = progress.querySelector('i');
    const resultsBox = pack.querySelector('.sp-results');
    const rows = new Map();
    ...
    function setMode(mode) {
      ...
      singleForm.style.display = bulk ? 'none' : '';
      if (pipeline) pipeline.style.display = bulk ? 'none' : '';
      if (singleStatus) singleStatus.style.display = bulk ? 'none' : '';
      if (singleResult) singleResult.style.display = bulk ? 'none' : '';
      if (details) details.style.display = bulk ? 'none' : '';
      pack.classList.toggle('is-active', bulk);
      title.textContent = bulk ? 'Expand Yomu in one pass' : singleTitle;
      headerCopy.textContent = bulk
        ? 'Paste multiple manga, manhwa, manhua or webtoon sites. Yomu tests each one and only adds sources it can actually read.'
        : singleCopy;
    ...
    switcher.addEventListener('click', (event) => {
      ...
      if (!button || runButton.disabled) return;
    ...
    runButton.addEventListener('click', async () => {
      ...
      if (!parsed.urls.length) {
        summary.textContent = parsed.invalid.length
          ? 'No valid public website URLs found.'
          : 'Paste some reading-site URLs first.';
        return;
      }
      ...
      let registry;
      try {
        const response = await fetch('/api/ext/sources', {
          cache: 'no-store',
      ...
      let completed = 0;
      const ready = [];
      const counters = { healthy: 0, browser: 0, skipped: parsed.invalid.length };

      const refreshSummary = () => {
        summary.textContent = `${completed}/${parsed.urls.length} tested · ${counters.healthy} healthy · ${counters.browser} browser-required · ${counters.skipped} skipped`;
        ...
        try {
          ...
          if (!extension) {
            ...
            if (!resolution.ready || !resolution.adapter) {
              const message = String(resolution.message || 'Yomu could not prove a complete reader path.');
              const browser = /browser|challenge|interactive|cloudflare/i.test(message);
              if (browser) {
                counters.browser += 1;
                setRow(target, 'browser', 'Browser', message);
              } else {
                counters.skipped += 1;
                setRow(target, 'blocked', 'Skipped', message);
              }
              return;
            }
          ...
          if (!adultAllowed() && isAdult(extension)) {
            counters.skipped += 1;
            setRow(target, 'blocked', '18+ hidden', 'Hidden by your current adult-content setting.');
            return;
          }

          setRow(target, 'working', 'Testing', `${extension.name || 'Source'} · checking catalog…`);
          const count = await verify(extension);
          ready.push({ target, extension, count });
          counters.healthy += 1;
          setRow(target, 'ready', 'Healthy', `${extension.name} · ${count} browse results sampled`);
        } catch (error) {
          counters.skipped += 1;
          setRow(target, 'failed', 'Failed', error?.message || 'Reader test failed.');
        } finally {
          completed += 1;
          refreshSummary();
        }
      ...
      async function worker() {
        while (true) {
          const index = cursor++;
          if (index >= parsed.urls.length) return;
      ...
      try {
        ...
        for (const item of ready) {
          ...
          if (state === 'added') setRow(item.target, 'added', 'Added', `${item.extension.name} · ready in Yomu`);
          else if (state === 'already') setRow(item.target, 'already', 'Already on', `${item.extension.name} · already enabled`);
          else setRow(item.target, 'failed', 'Failed', 'Could not save this source.');
        }
      } catch (error) {
        summary.textContent = error?.message || 'Could not save the healthy sources.';
      }

      const already = ready.length - added.changed;
      summary.textContent = `${added.changed} added · ${already} already enabled · ${counters.browser} browser-required · ${counters.skipped} skipped` + (parsed.truncated ? ` · max ${MAX_URLS} processed` : '');
