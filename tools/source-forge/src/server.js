...
app.post('/api/yomu/publish',async(req,res)=>{
  try{
    ...
    if(yomuUrl){
      try{
        const r=await fetch(new URL('/api/ext/refresh',yomuUrl),{headers:{Accept:'application/json'},signal:AbortSignal.timeout(15000)});
        refresh={ok:r.ok,status:r.status,body:await r.json().catch(()=>null)};
      }catch(e){refresh={ok:false,error:e.message}}
    }
