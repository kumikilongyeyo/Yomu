...
const STORE_DEFINITIONS: StoreDefinition[] = [
  {
    id: 'aidoku-yomu-community',
    name: 'Yomu Aidoku Sources',
    ecosystem: 'aidoku',
    url: 'https://smexhy.github.io/yomu-aidoku-sources/index.json',
    priority: 100,
  },
  {
    id: 'aidoku-community',
  ...
  },
  {
    ...
    priority: 100,
    directory: 'https://wotaku.wiki/ext/mihon',
  },
  {
    id: 'mihon-yuzono',
    name: 'Yuzono',
...
function normalizeInput(value: string): URL {
  ...
  if (!['https:', 'http:'].includes(url.protocol) || url.username || url.password) {
    throw new Error('Paste a public website URL.');
  }
...
function parseAidoku(store: StoreDefinition, document: any): FederatedMatch[] {
  ...
  return rows.flatMap((source: any) => {
    ...
    if (!baseUrl) return [];
    return [{
      ecosystem: 'aidoku' as const,
      storeId: store.id,
      storeName: store.name,
      name: String(source?.name ?? source?.id ?? normalizeHost(baseUrl)),
      baseUrl,
      id: stringValue(source?.id),
      version: stringValue(source?.version),
      artifact: stringValue(source?.downloadURL ?? source?.downloadUrl),
      language: Array.isArray(source?.languages) ? source.languages.join(',') : stringValue(source?.lang),
      runtimeHint: 'aidoku-wasm' as const,
  ...
  for (const ext of extensions) {
    ...
    for (const source of sources) {
      ...
      out.push({
        ...
        storeId: store.id,
        storeName: store.name,
        name: String(source?.name ?? ext?.name ?? normalizeHost(baseUrl)),
        baseUrl,
        id: stringValue(source?.id),
        version: stringValue(ext?.version ?? ext?.versionName ?? ext?.code),
        package: stringValue(ext?.pkg ?? ext?.packageName ?? ext?.package),
        artifact: stringValue(ext?.apk ?? ext?.downloadURL ?? ext?.downloadUrl),
        language: stringValue(source?.lang ?? ext?.lang),
        runtimeHint: 'mihon-android',
        priority: store.priority,
      });
    }
  }
...
function parseMangayomi(store: StoreDefinition, document: any): FederatedMatch[] {
  ...
  return rows.flatMap((source: any) => {
    ...
    return [{
      ...
      id: stringValue(source?.id),
      version: stringValue(source?.version),
      sourceCodeUrl: stringValue(source?.sourceCodeUrl),
      language: stringValue(source?.lang),
      hasCloudflare: Boolean(source?.hasCloudflare),
      runtimeHint: 'mangayomi-script' as const,
      priority: store.priority,
    }];
  });
}
...
export async function findFederatedSources(target: URL): Promise<{
  ...
  const settled = await Promise.allSettled(
    STORE_DEFINITIONS.map(async (store) => ({
      store,
      rows: parseStore(store, await fetchStore(store)),
    })),
  );
  ...
  for (const result of settled) {
    ...
    for (const row of result.value.rows) {
      if (hostMatch(row.baseUrl, targetHost)) all.push(row);
    }
  }

  const seen = new Set<string>();
  const matches = all
    .sort((a, b) => rankMatch(b, targetHost) - rankMatch(a, targetHost))
    .filter((row) => {
...
async function tryRuntimeBroker(env: Env, target: URL, matches: FederatedMatch[]): Promise<any | null> {
  ...
  const response = await fetch(`${config.url}/v1/resolve`, {
    ...
    body: JSON.stringify({
      ...
      implementations: matches.slice(0, 8).map(publicEvidence),
    }),
...
export async function handleFederatedResolve(
  request: Request,
  env: Env,
  url: URL,
  ...
  if (fallbackPayload.ready && fallbackPayload.adapter) {
  ...
  if (matches.length) {
    ...
    if (broker?.ready && broker?.adapter) {
      return json({
        ...
        confidence: broker.confidence ?? 'high',
        score: broker.score ?? 95,
        adapter: broker.adapter,
        evidence,
        federation,
        runtime: broker.runtime ?? best.runtimeHint,
        message: broker.message ?? `${best.name} is running through the remote ${best.runtimeHint} runtime.`,
      });
    }
    ...
    return json({
      ...
      ok: true,
      ready: false,
      route: 'store-federation',
      evidence,
      federation,
      runtime: best.runtimeHint,
      message: `Found ${best.name} in ${best.storeName}. Yomu knows the maintained implementation, but the remote ${best.runtimeHint} runner is not connected yet.`,
    });
  }
...
export async function handleStoreFederation(request: Request, env: Env, url: URL): Promise<Response> {
  if (url.pathname === '/api/fabric/stores/status') {
    ...
    return json({
      ...
      stores: STORE_DEFINITIONS.map(({ id, name, ecosystem, priority, directory }) => ({
        id,
        name,
        ecosystem,
        priority,
  ...
  if (url.pathname === '/api/fabric/stores/lookup') {
    ...
    try {
      ...
      return json({ error: error?.message ?? 'Invalid URL.' }, 400);
    }
    ...
    return json({
      ...
      ...result,
      matches: result.matches.map(publicEvidence),
    });
  }
