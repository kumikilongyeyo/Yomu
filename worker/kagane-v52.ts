...
const UA = 'Mozilla/5.0 (compatible; Yomu-Source-Fabric/5.2; +https://yomu.yomuread.workers.dev)';
...
async function kaganeJson(path: string, init: RequestInit = {}): Promise<any> {
  ...
  if (!response.ok) {
    const detail = cleanText(text).slice(0, 220);
    const error = new Error(`Kagane API returned HTTP ${response.status}${detail ? ` · ${detail}` : ''}`) as Error & { upstreamStatus?: number };
...
/** Mirrors the currently maintained Keiyoushi Kagane search payload. */
...
async function list(kind: 'popular' | 'latest' | 'search', q = '', page = 1) {
  ...
  const dto = await kaganeJson(`/search/series?${params}`, {
    method: 'POST',
...
async function integrity(force = false): Promise<Integrity> {
  ...
  const text = await response.text();
  if (!response.ok) throw new Error(`Kagane integrity endpoint returned HTTP ${response.status}${text ? ` · ${cleanText(text).slice(0, 160)}` : ''}`);
...
async function challenge(chapterId: string, force = false): Promise<AnyObject> {
  ...
  const text = await response.text();
  if (!response.ok) throw new Error(`Kagane reader returned HTTP ${response.status}${text ? ` · ${cleanText(text).slice(0, 160)}` : ''}`);
  return text ? JSON.parse(text) : {};
}
...
export async function handleKaganeV52(request: Request, url: URL): Promise<Response> {
  if (!url.pathname.startsWith('/api/fabric/source/kagane/')) return json({ error: 'Not a Kagane route.' }, 404);
  ...
  try {
    if (rest === 'series') return json(await list('popular', '', Number(url.searchParams.get('page') ?? '1') || 1), 200, 'private, max-age=180');
    if (rest === 'latest') return json(await list('latest', '', Number(url.searchParams.get('page') ?? '1') || 1), 200, 'private, max-age=120');
    if (rest === 'search') return json(await list('search', url.searchParams.get('q') ?? '', Number(url.searchParams.get('page') ?? '1') || 1), 200, 'private, max-age=60');

    const series = rest.match(/^series\/([^/]+)$/);
    if (series) return json(await detail(decodeURIComponent(series[1])), 200, 'private, max-age=120');
    ...
    if (manifest) {
      ...
      return json({
        ...
        chapterId,
        sourceSeriesId: '',
        manifestVersion: `kagane-v52-${chapterId}-${pageRows.length}`,
